...
Иттерация заключается в последовательном повороте вектора **v** вокруг базового вектора с заданным шагом. Поворот одновременно в две стороны.
    6.1 Иттерация до тех пор, пока расстояние от иттерируемого сайта до атома меньше [<b>a</b>](#na).
    6.2 Иттерация обрывается положительно, когда выполняется два условия:
...
7. Если удалось найти сайт, то мы осуществляем этот процесс диффузии безусловно (для экономии рессурсов). И формируем диммер, с тремя базисными векторами.
...
. Если удалось найти сайт, то мы осуществляем этот процесс диффузии безусловно (для экономии рессурсов). И формируем диммер, с тремя базисными векторами.
В случае отрицательного исхода, атом просто диффундирует в другое направление.
